...
# catch if the user's clicks on the windows upper-right 'X'
def ask_quit(self):
...
def first_run(self):
    ...
    with conn:
        ...
        if count < 1:
            cur.execute(
                """INSERT INTO tbl_studenttracker (col_fname,col_lname,col_fullname,col_phone,col_email, col_course) VALUES (?,?,?,?,?,?)""",
                ('Emmett', 'Espinosa', 'Emmett Espinosa', '111-111-1111', 'EEspinosa@email.com', 'Python'))
            conn.commit()
    conn.close()
...
def onSelect(self, event):
    ...
    with conn:
        cursor = conn.cursor()
        cursor.execute("""SELECT col_fname,col_lname,col_phone,col_email, col_course FROM tbl_studenttracker WHERE col_fullname = (?)""",
                       [value])
...
def addToList(self):
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (
            len(var_email) > 0) and (len(var_course) > 0):  # enforce the user to provide both names
        conn = sqlite3.connect('db_studenttracker.db')
        with conn:
            cursor = conn.cursor()
            # Check the database for existance of the fullname, if so we will alert user and disregard request
            cursor.execute("""SELECT COUNT(col_fullname) FROM tbl_studenttracker WHERE col_fullname = '{}'""".format(
...
def onUpdate(self):
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0) and (len(var_course) > 0):  # ensure that there is data present
        ...
        with conn:
            cur = conn.cursor()
            # count records to see the user's changes
            ...
            print(count2)
            cur.execute("""SELECT COUNT(col_course) FROM tbl_studenttracker WHERE col_course = '{}'""".format(var_course))
            ...
            if count == 0 or count2 == 0 or count3 == 0:  # if proposed changes are not already in the database, then proceed
                ...
                if response:
                    ...
                    with conn:
                        cursor = conn.cursor()
                        cursor.execute(
                            """UPDATE tbl_studenttracker SET col_phone = '{0}',col_email = '{1}',col_course = '{2}' WHERE col_fullname = '{3}'""".format(
                                var_phone, var_email, var_course, var_value))
                        onClear(self)
                        conn.commit()
                else:
